"""Quote defaults: gusset / punched retail add-ons per kg."""

from __future__ import annotations

import sqlalchemy as sa
from alembic import op

revision = "0021_qdefaults_extr_ft"
down_revision = "0020_js_cust_facing_desc"
branch_labels = None
depends_on = None


def upgrade() -> None:
    bind = op.get_bind()
    is_sqlite = bind.dialect.name == "sqlite"
    insp = sa.inspect(bind)
    existing_cols = {c["name"] for c in insp.get_columns("quote_defaults")}
    existing_checks = {c["name"] for c in insp.get_check_constraints("quote_defaults") if c.get("name")}

    if "extrusion_gusset_retail_per_kg" not in existing_cols:
        op.add_column(
            "quote_defaults",
            sa.Column(
                "extrusion_gusset_retail_per_kg",
                sa.Numeric(12, 4),
                nullable=False,
                server_default=sa.text("0.4"),
            ),
        )
    if "extrusion_punched_retail_per_kg" not in existing_cols:
        op.add_column(
            "quote_defaults",
            sa.Column(
                "extrusion_punched_retail_per_kg",
                sa.Numeric(12, 4),
                nullable=False,
                server_default=sa.text("0.2"),
            ),
        )

    missing_checks = [
        ("ck_quote_defaults_extrusion_gusset_retail_nonneg", "extrusion_gusset_retail_per_kg >= 0"),
        ("ck_quote_defaults_extrusion_punched_retail_nonneg", "extrusion_punched_retail_per_kg >= 0"),
    ]
    missing_checks = [(name, sql) for name, sql in missing_checks if name not in existing_checks]

    if not missing_checks:
        return

    if is_sqlite:
        # SQLite cannot ALTER constraints directly; use batch copy-and-move.
        with op.batch_alter_table("quote_defaults", recreate="always") as batch_op:
            for name, sqltext in missing_checks:
                batch_op.create_check_constraint(name, sqltext)
    else:
        for name, sqltext in missing_checks:
            op.create_check_constraint(name, "quote_defaults", sqltext)


def downgrade() -> None:
    bind = op.get_bind()
    is_sqlite = bind.dialect.name == "sqlite"
    insp = sa.inspect(bind)
    existing_cols = {c["name"] for c in insp.get_columns("quote_defaults")}
    existing_checks = {c["name"] for c in insp.get_check_constraints("quote_defaults") if c.get("name")}

    drop_checks = [
        "ck_quote_defaults_extrusion_punched_retail_nonneg",
        "ck_quote_defaults_extrusion_gusset_retail_nonneg",
    ]
    drop_checks = [n for n in drop_checks if n in existing_checks]
    drop_cols = ["extrusion_punched_retail_per_kg", "extrusion_gusset_retail_per_kg"]
    drop_cols = [n for n in drop_cols if n in existing_cols]

    if not drop_checks and not drop_cols:
        return

    if is_sqlite:
        with op.batch_alter_table("quote_defaults", recreate="always") as batch_op:
            for name in drop_checks:
                batch_op.drop_constraint(name, type_="check")
            for name in drop_cols:
                batch_op.drop_column(name)
    else:
        for name in drop_checks:
            op.drop_constraint(name, "quote_defaults", type_="check")
        for name in drop_cols:
            op.drop_column("quote_defaults", name)
