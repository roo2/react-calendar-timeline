...
revision = "0010_constraints_indexes_sequences"
down_revision = "0009_telemetry"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    if _is_postgres(conn):
        ...
        # Append-only protections where feasible (PostgreSQL triggers)
        op.execute(
            """
        ...
        # SQLite: create partial unique index via raw SQL; skip exclusion constraints and triggers
        op.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS uq_running_run_per_machine "
            "ON operation_runs(machine_id) WHERE status = 'running'"
        )
        # Sequences not applicable on SQLite
...
def downgrade() -> None:
    ...
    if _is_postgres(conn):
        ...
        # SQLite: drop partial unique index
        op.execute("DROP INDEX IF EXISTS uq_running_run_per_machine")

    # Do not drop extensions on SQLite
