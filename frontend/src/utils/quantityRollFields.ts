...
/**
 * Map a persisted or API `qty_type` string to {@link QtyType}.
 * Unknown values default to `units`.
 */
export function qtyTypeFromPersisted(raw: string | undefined | null): QtyType {
  const s = String(raw ?? '').trim().toLowerCase()
  if (s === 'kg') return 'kg'
  if (s === 'total_rolls') return 'total_rolls'
  if (s === 'rolls_units') return 'rolls_units'
  if (s === 'units') return 'units'
  return 'units'
}
